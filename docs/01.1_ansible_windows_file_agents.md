...
### Approve File Agent
as we could see from the Agent registration Status, the Agent´s name is the IP Address.  We need to Approve the Host with a whitelist entry Vaving the IP Adress from the Name Field.
