...
First, we need to upload a Package to Minio. We can do from ansible host:

```bash
ansible-playbook ~/workspace/0499/playbooks/s3_upload.yaml -e "package=dellemc-ppdm-upgrade-sw-19.17.0-5.pkg" -e "release=19.17"
```
